  ...
  async execute(interaction) {
    try {
      ...
      if (!currentMemberData) {
        ...
        const embed = embedBuilder.buildForUser(discordServer, i18n.__({ phrase: 'verify.link.messageTitle', locale: useLocale }), i18n.__({ phrase: 'verify.link.success', locale: useLocale }));
        await interaction.editReply({ embeds: [embed], ephemeral: true });
      } else {
        const embed = embedBuilder.buildForUser(discordServer, i18n.__({ phrase: 'verify.link.messageTitle', locale: useLocale }), i18n.__({ phrase: 'verify.link.notLinked', locale: useLocale }));
        await interaction.editReply({ embeds: [embed], ephemeral: true });
      }
    } catch (error) {
