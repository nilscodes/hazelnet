  ...
  async execute(interaction) {
    try {
      ...
      if (currentMemberData) {
        ...
        const embed = embedBuilder.buildForUser(discordServer, i18n.__({ phrase: 'verify.unlink.messageTitle', locale: useLocale }), i18n.__({ phrase: 'verify.unlink.success', locale: useLocale }));
        await interaction.editReply({ embeds: [embed], ephemeral: true });
      } else {
        const embed = embedBuilder.buildForUser(discordServer, i18n.__({ phrase: 'verify.unlink.messageTitle', locale: useLocale }), i18n.__({ phrase: 'verify.unlink.notLinked', locale: useLocale }));
        await interaction.editReply({ embeds: [embed], ephemeral: true });
      }
    } catch (error) {
