    ...
    try {
      ...
      if (parseInt(minimumTokenQuantity, 10) > 0) {
        if (maximumTokenQuantity === null || (parseInt(maximumTokenQuantity, 10) > 0 && parseInt(maximumTokenQuantity, 10) >= minimumTokenQuantity)) {
          ...
          const embed = embedBuilder.buildForAdmin(discordServer, '/configure-tokenroles add', i18n.__({ phrase: 'configure.tokenroles.add.errorMaximumTokens', locale: useLocale }), 'configure-tokenroles-add');
          await interaction.editReply({ embeds: [embed], ephemeral: true });
        }
      } else {
