  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (stakepoolToRemove) {
        ...
        const embed = embedBuilder.buildForAdmin(discordServer, '/configure-stakepool remove', i18n.__({ phrase: 'configure.stakepool.remove.success', locale: useLocale }), 'configure-stakepool-remove', [
          {
        ...
        const embed = embedBuilder.buildForAdmin(discordServer, '/configure-stakepool remove', i18n.__({ phrase: 'configure.stakepool.remove.errorNotFound', locale: useLocale }, { stakepoolHash }), 'configure-stakepool-remove');
        await interaction.editReply({ embeds: [embed], ephemeral: true });
      }
    } catch (error) {
